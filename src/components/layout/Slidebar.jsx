...
import React from 'react';
import { AiOutlineClose, AiOutlineDashboard, AiOutlineClockCircle, AiOutlineCalendar, AiOutlineLogout } from 'react-icons/ai';
import { CgProfile } from 'react-icons/cg';
import { NavLink } from 'react-router-dom';
import { FaUsers } from "react-icons/fa6";
...
export default function Slidebar({ isOpen, handleSlidebar, userEmail, onLogout }) {
  ...
  return (
    <>
      ...
      <div className={`h-screen w-64 bg-[#2C5282] dark:bg-gray-800 text-white fixed top-0 left-0 transform transition-transform duration-300 ease-in-out z-50 flex flex-col
      ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static`}> 
        
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-800 dark:border-gray-700">
          <FaUsers size={50} className="text-white mr-4"/>
          <h1 className="font-bold text-2xl">
            Employee Portal
          </h1>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) => 
                  isActive 
                    ? 'w-full flex items-center px-6 py-3 transition-colors bg-slate-800 dark:bg-gray-700 text-white border-l-4 border-[#2C5284] dark:border-blue-500' 
                    : 'w-full flex items-center px-6 py-3 transition-colors text-white hover:bg-slate-800 dark:hover:bg-gray-700'
                }
              >
                <Icon size={20} className="mr-3" />
                <span className="font-medium">{item.name}</span>
        ...
        {/* User Profile Section */}
        <div className="border-t border-slate-800 dark:border-gray-700 p-4">
          <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-800 dark:hover:bg-gray-700 transition-colors">
            <div className="w-10 h-10 rounded-full bg-slate-700 dark:bg-gray-600 flex items-center justify-center">
              <CgProfile size={24} className="text-white" />
            </div>
            <div className="flex-1 overflow-hidden">
              <p className="text-xs text-gray-300 dark:text-gray-400 truncate">{userEmail}</p>
            </div>
          </div>
          ...
          <button 
            onClick={handleLogoutClick}
            className="mt-2 w-full flex items-center justify-center gap-2 p-2 rounded-lg hover:bg-slate-800 dark:hover:bg-gray-700 bg-[#365F8D] dark:bg-gray-600 transition-colors text-sm cursor-pointer">
            <AiOutlineLogout size={18}/>
            <span className='mr-4 font-medium'>Logout</span>
          </button>
        </div>
